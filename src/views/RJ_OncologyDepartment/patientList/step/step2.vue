	...
	<div class="step2">
		...
		<el-alert
			...
			:title="`上传结果：共${Number(uploadNum.data.successNum) + Number(uploadNum.data.failNum)}条 |
				 成功 ${uploadNum.data.successNum}条 |
				 失败 ${uploadNum.data.failNum}条`"
			type="success"
			:closable="false">
		</el-alert>
		<el-tabs v-model="activeName" @tab-click="handleClick">
			...
			<el-tab-pane label="上传成功" name="first">
				<!-- <el-row class="finishjb">
					<el-col class="center" :span="24"><span class="red">疾病类型</span>信息</el-col>
				</el-row> -->
				<el-alert
					show-icon
					:closable="false"
					title="完善疾病类型的患者才能发起院后随访，请完善患者疾病类型信息"
					type="warning">
				</el-alert>
				<el-table border fit highlight-current-row :data="successTable" style="width: 100%" class="upTable" :loading="successLoading">
				...
				<el-row class="center" style = "padding-bottom: 50px;background: #fff;">
					<el-alert
						style="margin:10px 0"
						:closable="false"
						title="尚未来得及完善的患者信息，下次可到患者列表-未完善信息页面继续完善"
						type="warning">
					</el-alert>
			...
			<el-tab-pane label="上传失败" name="second">
				...
				<el-table border fit highlight-current-row :data="failTable" style="width: 100%" class="upTable" :loading="failLoading">
					<el-table-column prop="intime" label="入院时间" align="center"></el-table-column>
					<el-table-column prop="outtime" label="出院时间" align="center"></el-table-column>
